...
with tab1:
    ...
    if "bsr_categories" in df_ranked.columns and df_ranked["bsr_categories"].notna().any():
        ...
        # Einfügen vor cis_score
        idx = display_cols.index("cis_score")
        display_cols.insert(idx, "bsr_hauptkategorie")
        display_cols.insert(idx + 1, "bsr_subkategorie")
        rename_map["bsr_hauptkategorie"] = "BSR-Kategorie"
        rename_map["bsr_subkategorie"] = "BSR-Subkategorie"
    
    if has_trends:
        st.caption("📊 Δ-Spalten zeigen Veränderung zum vorherigen Scan (negativ = verbessert bei Rang/BSR, positiv = mehr bei Reviews)")
    
    st.dataframe(
        df_ranked[display_cols].rename(columns=rename_map),
